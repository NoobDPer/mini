...
import com.jk.minimalism.bean.dto.BeanField;
import com.jk.minimalism.bean.dto.GenerateInput;
import com.jk.minimalism.service.GenerateService;
import com.jk.minimalism.util.StrUtil;
import com.jk.minimalism.util.generate.TemplateUtil;
...
public class GenerateServiceImpl implements GenerateService {
    ...
    @Autowired
    public GenerateServiceImpl(JdbcTemplate jdbcTemplate) {
    ...
    public void saveCode(GenerateInput input) {
        String tableTypeSys = "0";
        if (tableTypeSys.equals(input.getTableType())) {
            // 字典表
            TemplateUtil.saveJavaDTO(input);
            TemplateUtil.saveSysXML(input);
        } else {
            // 数据表
